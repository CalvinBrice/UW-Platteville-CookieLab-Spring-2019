void setCartToOrigin() {
  ...
  if (analogRead(URead1) > 100) {
    ...
    while (!digitalRead(indSensorPin)); // While the inductive sensor is 0, do nothing
  ...
  if (!digitalRead(indSensorPin)) { // If cart not triggering inductive sensor
    digitalWrite(cartDirectionPin, HIGH); // Set the cart direction to forward
    analogWrite(trackPWM, HIGH); // Tell the cart to go
  }

  if (digitalRead(indSensorPin)) return; // If the cart is at the inductive sensor, exit function
